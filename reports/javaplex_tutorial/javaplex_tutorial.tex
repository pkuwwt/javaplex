...
\author{Andrew Tausz}
\email[A.~Tausz]{atausz@alumni.stanford.edu}
...
% TODO:  Andrew has example code showing how to display the simplices in a filtration. See:
% http://code.google.com/p/javaplex/source/browse/trunk/src/matlab/for_distribution/basic_examples/dump_example.m
% Add to tutorial

% TODO: Add javaplex_HA_files/getEndpoints_example.m. Can we return Inf instead of 2^512? See email on 5/15/12 to Violeta Kovacev-Nikolic, titled "JavaPlex: intervals"
...
% Ulrich Bauer at Snowbird wrote instructions for how to use javaPlex in mathematica. Figure this out, and add it to the tutorial? See Ulrich's MRC wiki page for instructions?
...
% TODO: coloring of wiki tutorial due to ' near "core1".
...
% Andrew has example code showing how to extract the Betti barcodes as Matlab vectors. See:
% http://code.google.com/p/javaplex/source/browse/trunk/src/matlab/for_distribution/basic_examples/endpoint_extraction_example.m
% Also see email thread from Mirko Lukovic on 6-23-2011.
% I don't think this needs to be in the tutorial, but maybe I will change my mind.
...
This tutorial is written for those using javaPlex with Matlab. However, one can run javaPlex without Matlab; see \url{http://code.google.com/p/javaplex/wiki/Interop}.
...
Please email Henry at \texttt{henrya@ima.umn.edu} or Andrew at \texttt{atausz@alumni.stanford.edu} if you have questions about this tutorial.
...
To install javaPlex for Matlab, go the the website \url{http://code.google.com/p/javaplex/downloads/list}. Download the zip file containing the Matlab examples. It should be called something like \texttt{matlab-examples\-4.1.0.tar.gz}. Extract the zip file. The resulting folder should be called \texttt{matlab-examples}. 
...
\begin{itemize}
\item \texttt{exercise\_1.m}
\item \texttt{exercise\_2.m}
\item \texttt{exercise\_3.m}
\item \texttt{exercise\_4.m}
\item \texttt{exercise\_5.m}
\item \texttt{exercise\_6.m}
\item \texttt{exercise\_8.m}
\item \texttt{exercise\_9.m}
\item \texttt{exercise\_16.m}
\item \texttt{exercise\_17.m}
\item \texttt{exercise\_18.m}
\item \texttt{exercise\_19.m}
\item \texttt{flatKleinDistanceMatrix.m}
\item \texttt{flatTorusDistanceMatrix.m}
\item \texttt{getDoubleTorusPoints.m}
\item \texttt{projPlaneDistanceMatrix.m}
\end{itemize}
...
You should change your current Matlab directory to \texttt{tutorial\_examples}, perhaps using the following command.
...
Our $5\times 5$ normalized patches are currently in the pixel basis: every coordinate corresponds to the range value at one of the 25 pixels. The Discrete Cosine Transform (DCT) basis is a useful basis for our patches \citep{Range, Mumford}. We change to this basis in order to plot a projection of the loop evidenced by Figure \ref{fig:rangeBetti}. The method \texttt{dct.m} returns the DCT change-of-basis matrix for square patches of size specified by the input parameter.
...
It is possible to display the simplices in a stream, along with their filtration times. For an example of how to do this, please see the file \href{http://code.google.com/p/javaplex/source/browse/trunk/src/matlab/for_distribution/basic_examples/dump_example.m}{\texttt{dump\_example.m}} in the folder {\texttt{basic\_examples}.
...
Several exercise solutions are accompanied by Matlab scripts, which are available in the folder \texttt{tutorial\_solutions}.
...
\begin{solution}
See the Matlab script \texttt{exercise\_1.m} in folder \texttt{tutorial\_solutions} for a solution. We use 9 vertices, which we think of as a $3 \times 3$ grid numbered as a telephone keypad. We identify opposite sides.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_2.m} for a solution. We use 9 vertices, which we think of as a $3 \times 3$ grid numbered as a telephone keypad. We identify opposite sides, with left and right sides identified with a twist. 
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_3.m} for a solution. We use the minimal triangulation for the projective plane, which contains 6 vertices.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_4.m} and the Matlab function \texttt{flatTorusDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_5.m} and the Matlab function \texttt{flatKleinDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_6.m} and the Matlab function \texttt{projPlaneDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_8.m} for a solution. Our planar dataset is 6 evenly spaced points on the unit circle. We build a Vietoris--Rips stream which, at the correct filtration value, is an octahedron. 
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_9.m} for a solution. Our planar dataset is 14 evenly spaced points on the unit circle. We build a Vietoris--Rips stream which, at the correct filtration value, is homeomorphic to the 6-sphere. It has 14 vertices because it is obtained by suspending the 0-sphere six times, for a total of $2 + (6 \times 2) = 14$ vertices. 
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_16.m} and the Matlab function \texttt{flatTorusDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_17.m} and the Matlab function \texttt{flatKleinDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_18.m} and the Matlab function \texttt{projPlaneDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{solution}
See the Matlab script \texttt{exercise\_19.m} and the Matlab function \texttt{getDoubleTorusPoints.m}. Thanks to Ulrich Bauer for this solution.
\end{solution}
